-- ContextRAG PostgreSQL Extension v1.0
-- Contextual Row-based RAG for enriched, explainable retrieval
...
-- Multiple embedding types per entity (base, local_context)
...
-- B-tree indexes on source columns and entity_type
CREATE INDEX idx_entities_source ON contextrag.entities(source_schema, source_table, source_id);
CREATE INDEX idx_entities_type ON contextrag.entities(entity_type);
CREATE INDEX idx_entities_active ON contextrag.entities(is_active) WHERE is_active = TRUE;
CREATE INDEX idx_entities_updated ON contextrag.entities(updated_at);

-- GIN index on metadata JSONB
CREATE INDEX idx_entities_metadata ON contextrag.entities USING GIN (metadata);

-- Entity context indexes
CREATE INDEX idx_entity_context_entity ON contextrag.entity_context(entity_id);
CREATE INDEX idx_entity_context_type ON contextrag.entity_context(context_type);
CREATE INDEX idx_entity_context_expires ON contextrag.entity_context(expires_at) WHERE expires_at IS NOT NULL;

-- HNSW indexes on embeddings (separate per embedding_type for semantic separation)
CREATE INDEX idx_embeddings_base_hnsw ON contextrag.entity_embeddings
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64)
    WHERE embedding_type = 'base';

CREATE INDEX idx_embeddings_local_context_hnsw ON contextrag.entity_embeddings
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64)
    WHERE embedding_type = 'local_context';

-- B-tree indexes on embedding metadata
CREATE INDEX idx_embeddings_entity ON contextrag.entity_embeddings(entity_id);
CREATE INDEX idx_embeddings_type ON contextrag.entity_embeddings(embedding_type);
...
BEGIN
    ...
    WHERE emb.embedding_type = p_embedding_type
      ...
      AND (p_entity_type IS NULL OR e.entity_type = p_entity_type)
      AND (
          -- If no context filters specified, include all
          (p_context_type IS NULL AND p_context_key IS NULL AND p_context_value IS NULL)
          OR
          -- Otherwise, only include entities matching context filters
          e.id IN (SELECT entity_id FROM matched_entities)
      )
    ORDER BY emb.embedding <=> p_query_embedding
    LIMIT p_limit;
END;
...
-- Find entities needing re-embedding (content changed since last embedding)
...
-- Get entity details including context and embeddings
